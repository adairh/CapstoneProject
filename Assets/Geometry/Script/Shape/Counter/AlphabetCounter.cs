...
public static class AlphabetCounter
{
    ...
    {
        var result = NumberToLabel(count);
        count++;
        return result;
    }
    ...
    {
        var cycle = num / 26; // Number of full A-Z cycles
        var letter = (char)('A' + num % 26); // Current letter
    ...
    public static int LabelToNumber(string label)
    {
        ...
        var letter = label[0];
        var baseValue = letter - 'A'; // Convert 'A' to 0, 'B' to 1, ..., 'Z' to 25

        var cycle = label.Length - 1; // Count number of apostrophes
