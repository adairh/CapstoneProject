...
/*public class LobbyUI : MonoBehaviour
{
    ...
    public void UpdateStatus(string message)
    {
        ...
        // Prevent duplicate pop-ups with a lock and cooldown
        if (isShowingPopup && (now - lastStatusTime) < statusRepeatCooldown)
        {
            Debug.Log($"Duplicate status ignored due to lock or cooldown: {message}");
            return;
        }
