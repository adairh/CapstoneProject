...
namespace OpenAI_API
{
    ...
    {
        private static APIAuthentication cachedDefault;
        ...
        /// <summary>
        ///     The default authentication to use when no other auth is specified.  This can be set manually, or automatically
        ///     loaded via environment variables or a config file.  <seealso cref="LoadFromEnv" />
        ///     <seealso cref="LoadFromPath(string, string, bool)" />
        /// </summary>
        ...
        /// <summary>
        ///     Attempts to load api key from environment variables, as "OPENAI_KEY" or "OPENAI_API_KEY".  Also loads org if from
        ///     "OPENAI_ORGANIZATION" if present.
        /// </summary>
        /// <returns>
        ///     Returns the loaded <see cref="APIAuthentication" /> any api keys were found, or <see langword="null" /> if
        ///     there were no matching environment vars.
        /// </returns>
        public static APIAuthentication LoadFromEnv()
        {
            ...
            var org = Environment.GetEnvironmentVariable("OPENAI_ORGANIZATION");
        ...
        /// <param name="searchUp">
        ///     Whether to recursively traverse up the directory tree if the <paramref name="filename" /> is not
        ///     found in the <paramref name="directory" />
        /// </param>
        /// <returns>
        ///     Returns the loaded <see cref="APIAuthentication" /> any api keys were found, or <see langword="null" /> if it
        ///     was not successful in finding a config (or if the config file didn't contain correctly formatted API keys)
        /// </returns>
        public static APIAuthentication LoadFromPath(string directory = null, string filename = ".openai",
            bool searchUp = true)
        {
            if (directory == null)
        ...
        /// <summary>
        ///     Tests the api key against the OpenAI API, to ensure it is valid.  This hits the models endpoint so should not be
        ///     charged for usage.
        /// </summary>
        /// <returns>
        ///     <see langword="true" /> if the api key is valid, or <see langword="false" /> if empty or not accepted by the
        ///     OpenAI API.
        /// </returns>
    ...
    {
	    /// <summary>
	    ///     A helper method to swap out <see langword="null" /> <see cref="APIAuthentication" /> objects with the
	    ///     <see cref="APIAuthentication.Default" /> authentication, possibly loaded from ENV or a config file.
	    /// </summary>
