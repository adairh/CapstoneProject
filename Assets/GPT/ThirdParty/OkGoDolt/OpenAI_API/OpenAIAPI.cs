...
namespace OpenAI_API
{
	...
	public class OpenAIAPI : IOpenAIAPI
    {
	    ...
	    ///     The name of your model deployment. You're required to first deploy a model before you can
	    ///     make calls.
	    /// </param>
	    /// <param name="apiKey">
	    ///     The API authentication information to use for API calls, or <see langword="null" /> to attempt to
	    ///     use the <see cref="APIAuthentication.Default" />, potentially loading from environment vars or from a config file.
	    ///     Currently this library only supports the api-key flow, not the AD-Flow.
	    /// </param>
	    /// <returns></returns>
	    public static OpenAIAPI ForAzure(string YourResourceName, string deploymentId, APIAuthentication apiKey = null)
        {
            ...
            api.ApiUrlFormat = $"https://{YourResourceName}.openai.azure.com/openai/deployments/{deploymentId}/" +
                               "{1}?api-version={0}";
            return api;
        }
    }
