...
namespace OpenAI_API.Models
{
	...
	public class ModelsEndpoint : EndpointBase, IModelsEndpoint
    {
        ...
        {
            var resultAsString = await HttpGetContent<JsonHelperRoot>($"{Url}/{id}");
	    ...
	    /// <returns>Asynchronously returns the <see cref="Model" /> with all available properties</returns>
	    [Obsolete("Use the overload without the APIAuthentication parameter instead, as custom auth is no longer used.",
            false)]
