...
namespace OpenAI_API.Chat
{
	...
	public class ChatEndpoint : EndpointBase, IChatEndpoint
    {
        ...
        {
            var request = new ChatRequest(DefaultChatRequestArgs)
            {
                Messages = messages,
        ...
        {
            return CreateChatCompletionAsync(userMessages.Select(m => new ChatMessage(ChatMessageRole.User, m))
                .ToArray());
        }
        ...
        public async Task StreamCompletionAsync(ChatRequest request, Action<int, ChatResult> resultHandler)
        {
            ...
            await foreach (var res in StreamChatEnumerableAsync(request)) resultHandler(index++, res);
        }
        ...
        {
            await foreach (var res in StreamChatEnumerableAsync(request)) resultHandler(res);
        }
