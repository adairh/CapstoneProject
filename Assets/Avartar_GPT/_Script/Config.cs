...
public class Config : MonoBehaviour
{
    ...
    private AIManager _aiManagerScript;
    private readonly List<Friend> _friendList = new();
    private string _loadedConfigFile = "";
    private readonly float m_version = 0.01f;
    ...
    {
        return m_version.ToString("0.00");
    }
    ...
    private string LoadConfigFromFile(string fileName)
    {
        var config = "";
    ...
    {
        var curFriendIndex = _aiManagerScript.GetFriendIndex();
        ...
        using (var reader = new StringReader(fileContents))
        {
            for (var line = reader.ReadLine(); line != null; line = reader.ReadLine())
            {
                ...
                else if (words[0] == "set_openai_api_key")
                {
                ...
                else if (words[0] == "set_elevenlabs_api_key")
                {
                    _aiManagerScript.SetElevenLabsAPIKey(words[1]);
                }
                else if (words[0] == "add_friend")
                {
                ...
                else if (words[0] == "set_friend_base_prompt")
                {
                    ...
                    var basePrompt = "";
                    for (var line2 = reader.ReadLine(); line2 != null; line2 = reader.ReadLine())
                ...
                else if (words[0] == "set_friend_language")
                {
                    friend._language = words[1].ToLower();
                }
                else if (words[0] == "set_friend_token_memory")
                {
                    //convert to int with TryParse
                    int.TryParse(words[1], out friend._friendTokenMemory);
                }
                else if (words[0] == "set_friend_max_tokens_to_generate")
                {
                    //convert to int with TryParse
                    int.TryParse(words[1], out friend._maxTokensToGenerate);
                }

                else if (words[0] == "set_friend_voice_pitch")
                {
                    //convert to int with TryParse
                    float.TryParse(words[1], out friend._pitch);
                }
                else if (words[0] == "set_friend_voice_speed")
                {
                    //convert to int with TryParse
                    float.TryParse(words[1], out friend._speed);
                }
                else if (words[0] == "set_friend_google_voice")
                {
                    //convert to int with TryParse
                    friend._googleVoice = words[1];
                }
                else if (words[0] == "set_friend_elevenlabs_voice")
                {
                    //convert to int with TryParse
                    friend._elevelLabsVoice = words[1];
                }
                else if (words[0] == "set_friend_visual")
                {
                    //convert to int with TryParse
                    friend._visual = words[1];
                }
                else if (words[0] == "set_friend_temperature")
                {
                    //convert to int with TryParse
                    float.TryParse(words[1], out friend._temperature);
                }
                else if (words[0] == "set_friend_elevenlabs_stability")
                {
                    //convert to int with TryParse
