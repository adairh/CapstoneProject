...
namespace CrazyMinnow.SALSA.OneClicks
{
    public class OneClickBase : MonoBehaviour
    {
        ...
        protected static List<SkinnedMeshRenderer> requiredSmrs = new();
        protected static List<OneClickConfiguration> oneClickConfigurations = new();
...
//		protected static Eyes eyes;
        ...
        protected static void NewExpression(string expressionName)
        {
            if (currentConfiguration.type == OneClickConfiguration.ConfigType.Salsa)
                currentConfiguration.oneClickExpressions.Add(new OneClickExpression(expressionName,
                    new List<OneClickComponent>()));
            else
                currentConfiguration.oneClickExpressions.Add(
                    new OneClickEmoterExpression(expressionName, new List<OneClickComponent>()));
        }
        ...
        {
            var currentConfigExpression =
                currentConfiguration.oneClickExpressions[currentConfiguration.oneClickExpressions.Count - 1];
            ...
            EmoteRepeater.StartDelay repeaterStartType = EmoteRepeater.StartDelay.Immediately)
        {
            var currentExpression =
                (OneClickEmoterExpression)currentConfiguration.oneClickExpressions[
                    currentConfiguration.oneClickExpressions.Count - 1];
        ...
        protected static void DoOneClickiness(GameObject go, AudioClip clip)
        {
            DoOneClickiness(go);
            Debug.LogWarning(
                "OneClickBase no longer accepts an AudioClip as a parameter for DoOneClickiness(). Please upgrade your OneClick to the latest version. Attempting to pass your AudioClip to the new Base functionality. NOTE: this may not meet your requirements.");
        ...
        public static void ConfigureSalsaAudioSource(GameObject go, AudioClip audioClip, bool useAudioSource)
        {
            ...
            if (salsaInstance)
            {
                ...
                {
                    Debug.Log(
                        "Per the applied OneClick, an AudioSource is not used, SALSA has been set to use External Analysis." +
        ...
        {
            for (var exp = 0; exp < configuration.oneClickExpressions.Count; exp++)
            {
                var expression = InitializeExpressionType(configuration, exp);

                var
                    componentCount =
                        0; // keeps track of good component creation to prevent creating components when criteria are not met (i.e. search blendshape name not found).
                ...
                for (var j = 0; j < configuration.oneClickExpressions[exp].components.Count; j++)
                {
                    ...
                    switch (oneClickComponent.type)
                    {
                        case OneClickComponent.ComponentType.Shape:
                            ...
                            for (var i = 0; i < requiredSmrs.Count; i++)
                            {
                                var isSpecificSmrProvided = !string.IsNullOrEmpty(oneClickShapeComponent.specificSmr);
                                var isSpecificComponentForWrongSmr = isSpecificSmrProvided &&
                                                                     !Regex.IsMatch(requiredSmrs[i].name,
                                                                         oneClickShapeComponent.specificSmr,
                                                                         RegexOptions.IgnoreCase);
                                if (isSpecificComponentForWrongSmr)
                                    continue;

                                var blendshapeIndex = -1;
                        ...
                        case OneClickComponent.ComponentType.Animator:
                            ...
                            if (animator == null)
                            {
                                Debug.LogWarning("Could not find Animator: " +
                                                 oneClickAnimatorComponent.componentSearchName);
                                continue;
                            }
                            ...
                            controlHelper.isTriggerParameterBiDirectional =
                                oneClickAnimatorComponent.isTriggerParmBiDirectional;
                ...
                if (componentCount == 0)
                {
                    Debug.Log("Removed expression " + expression.name +
                              " This may be OK, but may indicate a change in the model generator. If this is a supported model, contact Crazy Minnow Studio via assetsupport@crazyminnow.com.");
                    switch (configuration.type)
                    {
                        case OneClickConfiguration.ConfigType.Salsa:
        ...
        {
            var bNames = new string[smr.sharedMesh.blendShapeCount];
            for (var i = 0; i < smr.sharedMesh.blendShapeCount; i++) bNames[i] = smr.sharedMesh.GetBlendShapeName(i);
