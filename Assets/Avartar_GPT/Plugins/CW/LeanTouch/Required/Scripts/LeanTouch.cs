...
namespace Lean.Touch
{
	...
	///     You can access this data via Lean.Touch.LeanTouch.Instance.Fingers, or hook into the Lean.Touch.LeanTouch.On___
	///     events.
	///     NOTE: If you experience a one frame input delay you should edit your ScriptExecutionOrder to force this script to
	///     update before your other scripts.
	/// </summary>
	[ExecuteInEditMode]
    ...
    public class LeanTouch : MonoBehaviour
    {
        ...
        /// </summary>
        public static List<LeanFinger> Fingers = new(10);

        /// <summary>
        ///     This list contains all fingers that were once touching the screen. This is used to manage finger tapping, as
        ///     well as 'inactive' fingers that are so old they're no longer eligible for tapping.
        /// </summary>
        public static List<LeanFinger> InactiveFingers = new(10);
        ...
        /// <summary>
        ///     This gets fired when a finger has been touching the screen for longer than <b>TapThreshold</b> seconds,
        ///     causing it to be ineligible for the tap and swipe events.
        /// </summary>
        public static event Action<LeanFinger> OnFingerOld;

        /// <summary>
        ///     This gets fired when a finger taps the screen (this is when a finger begins and stops touching the screen
        ///     within the 'TapThreshold' time).
        /// </summary>
        public static event Action<LeanFinger> OnFingerTap;

        /// <summary>
        ///     This gets fired when a finger swipes the screen (this is when a finger begins and stops touching the screen
        ///     within the 'TapThreshold' time, and also moves more than the 'SwipeThreshold' distance) (LeanFinger = The current
        ///     finger)
        /// </summary>
        ...
        /// <summary>
        ///     This gets fired after a finger has stopped touching the screen for longer than <b>TapThreshold</b> seconds,
        ///     making it ineligible for any future taps. This can be used to detect when you've done a single tap instead of a
        ///     double tap, etc.
        /// </summary>
        ...
        /// <summary>
        ///     This will be invoked when it's time to simulate fingers. You can call the <b>AddFinger</b> method to simulate
        ///     them.
        /// </summary>
        ...
        public static float CurrentTapThreshold =>
            Instances.Count > 0 ? Instances[0].tapThreshold : DEFAULT_TAP_THRESHOLD;

        /// <summary>
        ///     This allows you to set how many pixels of movement (relative to the ReferenceDpi) are required within the
        ///     TapThreshold for a swipe to be triggered.
        /// </summary>
        ...
        public static float CurrentSwipeThreshold =>
            Instances.Count > 0 ? Instances[0].swipeThreshold : DEFAULT_SWIPE_THRESHOLD;
		...
		public float ReclaimThreshold { set { reclaimThreshold =
 value; } get { return reclaimThreshold; } } [SerializeField] private float reclaimThreshold =
 DEFAULT_RECLAIM_THRESHOLD;
	    ...
	    /// <summary>
	    ///     This allows you to set the default DPI you want the input scaling to be based on. For example, if you set this
	    ///     to 200 and your display has a DPI of 400, then the <b>ScaledDelta</b> finger value will be half the distance of the
	    ///     pixel space <b>ScreenDelta</b> value.
	    /// </summary>
        ...
        public static int CurrentReferenceDpi =>
            Instances.Count > 0 ? Instances[0].referenceDpi : DEFAULT_REFERENCE_DPI;
        ...
        [SerializeField] private LayerMask guiLayers = DEFAULT_GUI_LAYERS;

        public static LayerMask CurrentGuiLayers => Instances.Count > 0 ? Instances[0].guiLayers : DEFAULT_GUI_LAYERS;
        ...
        /// <summary>
        ///     When using the old/legacy input system, by default it will convert touch data into mouse data, even if there
        ///     is no mouse. Enabling this setting will disable this behavior.
        /// </summary>
        ...
        private static readonly List<LeanFinger> filteredFingers = new(10);
        ...
        /// <summary>
        ///     If you multiply this value with any other pixel delta (e.g. ScreenDelta), then it will become device
        ///     resolution independent relative to the device DPI.
        /// </summary>
        ...
        /// <summary>
        ///     If you multiply this value with any other pixel delta (e.g. ScreenDelta), then it will become device
        ///     resolution independent relative to the screen pixel size.
        /// </summary>
        ...
        public static EventSystem GetEventSystem()
        {
            ...
            if (currentEventSystem == null) currentEventSystem = CwHelper.FindAnyObjectByType<EventSystem>();
        ...
        public static List<LeanFinger> GetFingers(bool ignoreIfStartedOverGui, bool ignoreIfOverGui,
            int requiredFingerCount = 0, bool ignoreHoverFinger = true)
        {
            ...
            {
                // Ignore?
                if (ignoreIfStartedOverGui && finger.StartedOverGui) continue;

                if (ignoreIfOverGui && finger.IsOverGui) continue;

                if (ignoreHoverFinger && finger.Index == HOVER_FINGER_INDEX) continue;
        ...
        private static readonly LeanFinger simulatedTapFinger = new();
        ...
        /// <summary>
        ///     You can call this method if you want to stop all finger events. You can then disable this component to prevent
        ///     new ones from updating.
        /// </summary>
        public void Clear()
        {
        ...
        private static readonly HashSet<LeanFinger> missingFingers = new();

        private static readonly List<LeanFinger> tempFingers = new();

        // Read new hardware finger data
        private void PollFingers()
        {
            // Submit real fingers?
            if (useTouch && CwInput.GetTouchCount() > 0)
            ...
            if (useHover && CwInput.GetMouseExists())
            {
            ...
            if (useMouse && CwInput.GetMouseExists())
            {
                var mouseSet = false;
                var mouseUp = false;
        ...
        public LeanFinger AddFinger(int index, Vector2 screenPosition, float pressure, bool set)
        {
            ...
            if (recordFingers)
            {
                ...
                else if (finger.Index == HOVER_FINGER_INDEX)
                {
                    ...
                    {
                        var removeCount =
                            LeanSnapshot.GetLowerIndex(finger.Snapshots, finger.Age - DEFAULT_RECORD_LIMIT);
...
namespace Lean.Touch.Editor
{
    ...
    {
        private static readonly List<LeanFinger> allFingers = new();
        ...
        public static event Action<TARGET> OnExtendInspector;
        ...
        protected override void OnInspector()
        {
            ...
            if (OnExtendInspector != null) OnExtendInspector.Invoke(tgt);
        ...
        private void DrawSettings(LeanTouch touch)
        {
            ...
            Draw("useMouse",
                "Should any mouse button press be stored as a finger?\n\nNOTE: It will be given a finger <b>Index</b> of MOUSE_FINGER_INDEX = -1.");
            Draw("useSimulator",
                "Should components hooked into the <b>OnSimulateFingers</b> event be used? (e.g. LeanTouchSimulator)");

            Separator();

            Draw("disableMouseEmulation", ref updateMouseEmulation,
            ...
            if (updateMouseEmulation) Each(tgts, t => t.UpdateMouseEmulation(), true);
        }

        private void DrawFingers(LeanTouch touch)
        {
            EditorGUILayout.LabelField(new GUIContent("Fingers", "Index - State - Taps - X, Y - Age"),
            ...
            for (var i = 0; i < allFingers.Count; i++)
            {
                ...
                if (style.normal.textColor.a > 0.0f)
                {
                    ...
                    if (finger.Down) state = "DOWN";
                    if (finger.Up) state = "UP";
                    if (finger.IsActive == false) state = "INACTIVE";
                    if (finger.Expired) state = "EXPIRED";

                    EditorGUILayout.LabelField(
                        finger.Index + " - " + state + " - " + finger.TapCount + "  + " +
                        Mathf.FloorToInt(screenPosition.x) + ", " + Mathf.FloorToInt(screenPosition.y) + ") - " +
