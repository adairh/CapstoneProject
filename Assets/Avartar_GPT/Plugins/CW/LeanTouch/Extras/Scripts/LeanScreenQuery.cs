...
namespace Lean.Common
{
    ...
    public struct LeanScreenQuery
    {
        ...
        private static List<KeyValuePair<GameObject, int>> tempLayers = new();
        ...
        public static void ChangeLayers(GameObject root, bool ancestors, bool children)
        {
            ...
            if (ancestors && root.transform.parent != null) ChangeLayers(root.transform.parent.gameObject, true, false);
        ...
        public T Query<T>(GameObject gameObject, Vector2 screenPosition)
        {
            ...
            if (TryQuery(gameObject, screenPosition, ref result, ref root, ref worldPosition)) return result;

            return default;
        }

        public bool TryQuery<T>(GameObject gameObject, Vector2 screenPosition, ref T result, ref Component root,
        ...
        {
            if (TryGetComponent(hit, ref result, ref component)) return true;
        ...
        private bool TryGetComponentInChildren<T>(Component hit, ref T result, ref Component component)
        {
            ...
            for (var i = 0; i < hit.transform.childCount; i++)
                if (TryGetComponentInParent(hit.transform.GetChild(i), ref result, ref component))
                    return true;
        ...
        private void DoRaycast3D(Camera camera, Vector2 screenPosition, ref Component bestResult,
            ref float bestDistance, ref Vector3 bestPosition)
        {
        ...
        private void DoRaycast2D(Camera camera, Vector2 screenPosition, ref Component bestResult,
            ref float bestDistance, ref Vector3 bestPosition)
        {
            var ray = camera.ScreenPointToRay(screenPosition);
        ...
        private void DoRaycastUI(Vector2 screenPosition, ref Component bestResult, ref float bestDistance,
...
namespace Lean.Common.Editor
{
    ...
    public class LeanScreenQuery_Drawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect rect, SerializedProperty property, GUIContent label)
        {
            ...
            DrawProperty(ref rect, property, label, "Method", label.text,
                "The method used to search the scene based on a screen position.\n\nRaycast = 3D, 2D, and EventSystem raycast.");

            EditorGUI.indentLevel++;
            {
                DrawProperty(ref rect, property, label, "Camera", null,
                ...
                switch (method)
                {
                    case LeanScreenQuery.MethodType.Raycast:
                    {
                        ...
                        DrawProperty(ref rect, property, label, "Layers", "Layers",
                            "The scene will be queried (e.g. Raycast) against these layers.");
                        CwEditor.EndError();
                        DrawProperty(ref rect, property, label, "Search", "Search",
                            "When the query hits a GameObject, how should the desired component be searched for relative to it?");
                        DrawProperty(ref rect, property, label, "RequiredTag", "RequiredTag",
            ...
            where T : Object
        {
            ...
            if (oldValue != newValue) propertyObject.objectReferenceValue = newValue;
