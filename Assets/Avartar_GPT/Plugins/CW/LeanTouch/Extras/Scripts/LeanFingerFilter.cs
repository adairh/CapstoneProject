...
{
	/// <summary>
	///     This class manages a list of fingers, and can return a filtered version of them based on the criteria you specify.
	///     This allows you to quickly implement complex controls involving multiple fingers.
	///     By default, all fingers seen by LeanTouch are used by this class, but you can set <b>Filter</b> to
	///     <b>ManuallyAddedFingers</b>, and you can manually call the <b>AddFinger</b> method to add them yourself.
	///     NOTE: If you use this class then you must call the <b>UpdateAndGetFingers</b> method every frame/Update to update
	///     the class state. This is required because this update method will remove fingers that went up. If you don't call
	///     this then they will remain, and this may lead to unexpected behavior.
	/// </summary>
	[Serializable]
    public class LeanFingerFilter
    {
        ...
        [NonSerialized] private List<LeanFinger> filteredFingers;

        [NonSerialized] private List<LeanFinger> fingers;

        public LeanFingerFilter(bool newIgnoreStartedOverGui) : this(default, newIgnoreStartedOverGui, default, default,
            default)
        {
        }

        public LeanFingerFilter(FilterType newFilter, bool newIgnoreStartedOverGui, int newRequiredFingerCount,
            int newRequiredMouseButtons, LeanSelectable newRequiredSelectable)
        {
            Filter = newFilter;
        ...
        /// <summary>
        ///     If the current RequiredSelectable is null, this method allows you to try and set it based on the specified
        ///     GameObject.
        /// </summary>
        ...
        {
            if (filteredFingers == null) filteredFingers = new List<LeanFinger>();

            filteredFingers.Clear();

            if (Filter == FilterType.AllFingers)
                filteredFingers.AddRange(
                    LeanSelectableByFinger.GetFingers(IgnoreStartedOverGui, false, 0, RequiredSelectable));
            else if (fingers != null) filteredFingers.AddRange(fingers);
            ...
            if (ignoreUpFingers)
                for (var i = filteredFingers.Count - 1; i >= 0; i--)
                    if (filteredFingers[i].Up)
                        filteredFingers.RemoveAt(i);

            if (RequiredMouseButtons > 0 && SimulatedFingersExist(filteredFingers))
            {
                for (var i = 0; i < 5; i++)
                {
                    var mask = 1 << i;

                    if ((RequiredMouseButtons & mask) != 0 && CwInput.GetMouseIsHeld(i) == false &&
                        CwInput.GetMouseWentUp(i) == false) filteredFingers.Clear();
                }

                return filteredFingers;
            }

            if (RequiredFingerCount > 0 && filteredFingers.Count != RequiredFingerCount) filteredFingers.Clear();
...
namespace Lean.Touch.Editor
{
    ...
    public class LeanFingerFilter_Drawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect rect, SerializedProperty property, GUIContent label)
        {
            ...
            DrawProperty(ref rect, property, label, "Filter", label.text,
                "The method used to find fingers to use with this component.\n\nManuallyAddedFingers = You must manually call the AddFinger function (e.g. from a UI button).");
            ...
            EditorGUI.indentLevel++;
            {
                switch (filter)
                {
                    case LeanFingerFilter.FilterType.AllFingers:
                    {
                        DrawProperty(ref rect, property, label, "RequiredSelectable", null,
                            "If the specified RequiredSelectable component's IsSelected setting is false, ignore all fingers?");
                        DrawProperty(ref rect, property, label, "RequiredFingerCount", null,
                            "If the amount of fingers doesn't match this number, ignore all fingers?\n\n0 = Any amount.");
                        DrawProperty(ref rect, property, label, "RequiredMouseButtons", null,
                            "When using simulated fingers, should a specific combination of mouse buttons be held?\n\n0 = Any.\n1 = Left.\n2 = Right.\n3 = Left + Right.\n4 = Middle.\n5 = Left + Middle.\n6 = Right + Middle.\n7 = Left + Right + Middle.");
                        DrawProperty(ref rect, property, label, "IgnoreStartedOverGui", null,
        ...
        private void DrawProperty(ref Rect rect, SerializedProperty property, GUIContent label, string childName,
            string overrideName = null, string overrideTooltip = null)
        {
