...
namespace CW.Common
{
    ...
    {
        private static readonly Stack<Random.State> seedStates = new();
        ...
        public static bool IsURP
        {
            get
            {
                ...
                if (crp != null)
                {
                    ...
                    if (title.Contains("Universal")) return true;
                }
        ...
        public static bool IsHDRP
        {
            get
            {
                ...
                if (crp != null)
                {
                    ...
                    if (title.Contains("HighDefinition")) return true;
                }
        ...
        {
            if (convertToGamma) color = ToGamma(color);
        ...
        public static void Resize<T>(List<T> list, int size)
        {
            ...
            else
            {
                ...
                for (var i = list.Count; i < size; i++) list.Add(default);
            }
        }
        ...
        {
            if (QualitySettings.activeColorSpace == ColorSpace.Linear) return Mathf.Pow(gamma, 1.0f / 2.2f);
        ...
        {
            if (QualitySettings.activeColorSpace == ColorSpace.Linear) return Mathf.Pow(linear, 2.2f);
        ...
        public static void SetTempMaterial(List<Material> materials)
        {
            ...
            if (materials != null) tempMaterials.AddRange(materials);
        }
        ...
        public static Material CreateTempMaterial(string materialName, string shaderName)
        {
            ...
            if (shader == null) Debug.LogError("Failed to find shader: " + shaderName);
...
#if UNITY_EDITOR
            if (recordUndo != null) Undo.RegisterCreatedObjectUndo(gameObject, recordUndo);
#endif
        ...
        {
            if (v >= -1.0f && v <= 1.0f) return (float)Math.Acos(v);
        ...
        {
            if (v >= -1.0 && v <= 1.0) return Math.Acos(v);
    ...
    public static partial class CwHelper
    {
            ...
            Vector4 coords, Color color)
        {
            if (shapeTexture != null)
            {
                ...
                if (cachedShapeOutline.SetPass(0)) Graphics.DrawMeshNow(GetQuadMesh(), shapeMatrix);
            }
        }
            ...
            where T : Object
        {
            ...
            if (string.IsNullOrEmpty(path) == false) return AssetDatabase.AssetPathToGUID(path);
            ...
            where T : Object
        {
            if (string.IsNullOrEmpty(guid) == false)
            {
                ...
                if (string.IsNullOrEmpty(path) == false) return AssetDatabase.LoadAssetAtPath<T>(path);
            }
