...
namespace Lean.Gui
{
    ...
    public class LeanSwitch : MonoBehaviour
    {
        ...
        public static LinkedList<LeanSwitch> Instances = new();
        ...
        public void Switch(int newState)
        {
            if (states != null && newState >= 0 && newState < states.Count)
                if (state != newState)
                {
                    ...
                    if (stateTransitions != null) stateTransitions.Begin();

                    if (changedStateTransitions != null) changedStateTransitions.Begin();

                    if (onChangedState != null) onChangedState.Invoke(state);
                }
        }
        ...
        public static void SwitchAll(string name, int state)
        {
            ...
            for (var i = Instances.Count - 1; i >= 0; i--)
            {
                ...
                if (instance.name == name) instance.Switch(state);
...
namespace Lean.Gui.Editor
{
    ...
    public class LeanSwitch_Editor : CwEditor
    {
        private void DrawState(TARGET tgt, TARGET[] tgts)
        {
            ...
            if (EditorGUI.EndChangeCheck()) Each(tgts, t => t.State = state);
        }
