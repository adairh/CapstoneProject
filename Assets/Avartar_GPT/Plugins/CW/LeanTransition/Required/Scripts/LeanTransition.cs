...
namespace Lean.Transition
{
    ...
    public class LeanTransition : MonoBehaviour
    {
        ...
        public static List<LeanTransition> Instances = new();

        private static readonly List<LeanState> unscaledUpdateStates = new();

        private static readonly List<LeanState> unscaledLateUpdateStates = new();

        private static readonly List<LeanState> unscaledFixedUpdateStates = new();

        private static readonly List<LeanState> updateStates = new();

        private static readonly List<LeanState> lateUpdateStates = new();

        private static readonly List<LeanState> fixedUpdateStates = new();

        private static readonly List<LeanMethod> tempBaseMethods = new();

        private static readonly List<LeanMethod> baseMethodStack = new();

        private static readonly Dictionary<string, Type> aliasTypePairs = new();
        ...
        public static LeanTiming CurrentDefaultTiming
        {
            get
            {
                if (Instances.Count > 0) return Instances[0].defaultTiming;
        ...
        public static int Count => unscaledUpdateStates.Count + unscaledLateUpdateStates.Count +
                                   unscaledFixedUpdateStates.Count + updateStates.Count + lateUpdateStates.Count +
        ...
        ///     current animation.
        /// </summary>
        public static Dictionary<string, Object> CurrentAliases { get; } = new();
        ...
        {
            if (current == LeanTiming.Default) current = LeanTiming.UnscaledUpdate;
        ...
        {
            if (root != null) InsertTransitions(root.transform, speed);
        }
        ...
        public static void InsertTransitions(Transform root, float speed = 1.0f, LeanState parentHead = null)
        {
            if (root != null)
            {
                ...
                for (var i = min; i < max; i++) baseMethodStack[i].Register();

                baseMethodStack.RemoveRange(min, max - min);

                var childParentHead = PreviousState;

                for (var i = 0; i < root.childCount; i++) InsertTransitions(root.GetChild(i), 1.0f, childParentHead);
            ...
            where T : LeanState, new()
        {
            ...
            if (Instances.Count == 0) new GameObject("LeanTransition").AddComponent<LeanTransition>();
        ...
        public static LeanState Register(LeanState state, float duration)
        {
            ...
            if (CurrentSpeed > 0.0f) state.Duration /= CurrentSpeed;
            ...
            if (OnRegistered != null) OnRegistered(state);
            ...
            if (currentConflict != LeanState.ConflictType.None)
            {
                ...
                for (var i = 0; i < currentIndex; i++)
                {
                        ...
                        transition.GetTarget() == currentTarget)
                    {
                        ...
                        if (currentConflict == LeanState.ConflictType.Complete) transition.Update(1.0f);
                    }
                }
            }
        }
        ...
        private void UpdateAll(List<LeanState> states, float delta)
        {
            ...
            for (var i = states.Count - 1; i >= 0; i--)
            {
                ...
                if (state.Prev.Count == 0)
                {
                    ...
                    {
                        if (state.Ignore == false) state.Update(state.Age / state.Duration);
        ...
        private static void FinishState(LeanState state)
        {
            ...
            for (var j = state.Next.Count - 1; j >= 0; j--) state.Next[j].Prev.Remove(state);

            state.Next.Clear();

            // Make sure we call update one final time with a progress value of exactly 1.0
            if (state.Ignore == false) state.Update(1.0f);
