...
public class DemoCube : MonoBehaviour
{
    ...
    public Text m_quantityLabel; // Text label for showing amount of instantiating game objects
    ...
    private void CreateGameObjects()
    {
        ...
        for (var t_count = 0; t_count < m_quantity; ++t_count)
        {
            // Create a new cube game object
            var t_newCube = GameObject.CreatePrimitive(PrimitiveType.Cube);
            // Change the cube's position
            t_newCube.transform.position +=
                new Vector3(Mathf.Sin(t_count * (360 / m_quantity) * Mathf.Deg2Rad) * t_radius, 3,
                    Mathf.Cos(t_count * (360f / m_quantity) * Mathf.Deg2Rad) * t_radius);
            // Add a rigidbody component to the cube to get it effected by physics
            t_newCube.AddComponent<Rigidbody>();
            // Create a new material
            var t_newColor = new Material(gameObject.GetComponent<Renderer>().material);
